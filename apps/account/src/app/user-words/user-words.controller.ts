...
export class UserWordsController {
  ...
  async addWord(@Payload() payload: UserWordPayload & { userId: number }): Promise<{ success: boolean; message: string }> {
    return this.userWordsService.addWord(payload.userId, payload.book_word_id, payload.type);
  }

  @MessagePattern(USER_WORDS.REMOVE)
  async removeWord(@Payload() payload: UserWordPayload & { userId: number }): Promise<{ success: boolean; message: string }> {
